const func = {
    ...
    loadInventory() {
        ...
        if (empty) {
            ...
            p.id = 'emptyInventory';
            document.querySelector('#inventory ul:first-of-type').append(p);
    ...
    doNotOverflow() {
        let h = jsonData.player.health;
        if (h > 15) jsonData.player.health = 15;
        if (h < -1) jsonData.player.health = -1;

        let hu = jsonData.player.hunger;
        if (hu > 20) jsonData.player.hunger = 20;
        if (hu < -1) jsonData.player.hunger = -1;

        let t = jsonData.player.thirst;
        if (t > 15) jsonData.player.thirst = 15;
        if (t < -1) jsonData.player.thirst = -1;

        let f = jsonData.player.fire;
        if (f > 3) jsonData.player.fire = 3;
        if (f < -1) jsonData.player.fire = -1;
    },
    ...
    activateButtons(button) {
        ...
        if (jsonData.inventory.wood > 19 && jsonData.inventory.rope > 9 && jsonData.inventory.pole == 0) document.querySelector('#poleBtn').className = 'activeButton';
        if (jsonData.inventory.rope > 19 && jsonData.inventory.sail == 0) document.querySelector('#sailBtn').className = 'activeButton';
    },

    // Gibt den Buttons die Klasse inactiveButton wenn nicht genug Materialien vorhanden sind
    deactivateButtons(button) {
        if (jsonData.inventory.wood < 1 && document.querySelector('#fireBtn').className != 'unactivatedButton') document.querySelector('#fireBtn').className = 'inactiveButton';
        if (jsonData.inventory.fish < 1 && document.querySelector('#fishBtn').className != 'unactivatedButton') document.querySelector('#fishBtn').className = 'inactiveButton';
        if (jsonData.inventory.coco < 1 && document.querySelector('#cocoBtn').className != 'unactivatedButton') document.querySelector('#cocoBtn').className = 'inactiveButton';
        if (jsonData.inventory.spear < 1 && document.querySelector('#huntBtn').className != 'unactivatedButton') document.querySelector('#huntBtn').className = 'inactiveButton';
        if ((jsonData.inventory.stone < 1 || jsonData.inventory.sticks < 1) && document.querySelector('#spearBtn').className != 'unactivatedButton') document.querySelector('#spearBtn').className = 'inactiveButton';
        if (jsonData.inventory.wood < 3 && document.querySelector('#plankBtn').className != 'unactivatedButton') document.querySelector('#plankBtn').className = 'inactiveButton';
        if (jsonData.inventory.leaves < 5 && document.querySelector('#ropeBtn').className != 'unactivatedButton') document.querySelector('#ropeBtn').className = 'inactiveButton';
        if ((jsonData.inventory.wood < 20 || jsonData.inventory.rope == 1 || jsonData.inventory.pole == 1) && document.querySelector('#poleBtn').className != 'unactivatedButton') document.querySelector('#poleBtn').className = 'inactiveButton';
        if ((jsonData.inventory.rope < 20 || jsonData.inventory.sail == 1 || jsonData.inventory.sail == 1) && document.querySelector('#sailBtn').className != 'unactivatedButton') document.querySelector('#sailBtn').className = 'inactiveButton';
    },
    ...
    checkForCorpses() {
        ...
        if (jsonData.player.rounds > 300) func.die('old');
    },
    ...
    getAttacked() {
        ...
        if (jsonData.player.rounds > 10) { // Habe ich hinzugefügt, da ich beim testweisen spielen bemerkt habe dass es echt Mist ist, wenn in der ersten Runde schon ein Angriff stattfindet
            if (jsonData.player.fire > 1) random = createNumber(0, 40)
    ...
    escapeIsland() {
        if (jsonData.inventory.sail > 0 && jsonData.inventory.pole > 0 && jsonData.inventory.plank > 19 && jsonData.inventory.rope > 9 && jsonData.inventory.fish > 2 && jsonData.inventory.coco > 2) {
            return ('Du hast alles, was du für deine Flucht von dieser Insel benötigst. Du legst die Bretter aus, bindest sie zusammen, befestigst dein Segel am Mast und stellst ihn auf. Darunter finden Fische und Kokosnüsse für die Reise Platz.<br><br>Es kann losgehen! Du schiebst das Floß mit Anlauf ins Wasser und springst drauf, als du nicht mehr stehen kannst. Du hisst das Segel.<br><br>Drei Tage später...<br><br>Am Horizont erblickst du einen Leuchtturm. Du hast es geschafft! Du bist frei!<br><br><br>Das Spiel hat dir hoffentlich viel Spaß gemacht. Aber es gibt etwas, das keinen Spaß macht: Tatsächlich mit einem klapprigen Boot über das Meer flüchten zu müssen.<br>Allein im Mittelmeer sind in den letzten 10 Jahren über 28.000 Menschen bei dem Versuch, Krieg, Hunger und bitterer Armut zu entkommen, ertrunken.<br>Die UNO Flüchtlingshilfe versucht diese Zustände zu verbessern. Mehr Infos findest du unter <a href="https://www.uno-fluechtlingshilfe.de/hilfe-weltweit/mittelmeer" target="_blank">uno-fluechtlingshilfe.de.</a><br><br><button id="again" class="activeButton">Nochmal spielen</button>');
            // Ich habe es leider auf die Schnelle nicht mehr geschafft, diesen Part umzubauen, ohne dass es mir 10 andere Dinge zerschießt :(
        };
    },
    ...
    warn() {
        ...
        if (jsonData.player.rounds > 250) {
            if (!func.warnedAge2) {
                func.warnedAge2 = true;
    ...
    newStoryPart(text) {
        ...
        if (document.querySelector('#again') != null) document.querySelector('#again').addEventListener('click', () => {
            localStorage.clear();
            location.reload();
        });
    ...
    // Fisch jagen
    huntFish() {
        const addedFish = createNumber(0, 4);
        let output;
